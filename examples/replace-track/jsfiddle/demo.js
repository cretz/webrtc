...
pc.oniceconnectionstatechange = e => console.debug('connection state change', pc.iceConnectionState)
...
pc.ontrack = event => {
  ...
  document.getElementById('video1').srcObject = new MediaStream([event.track])
...
const streams = [canvasOne.captureStream(), canvasTwo.captureStream()]
streams.forEach(stream => stream.getVideoTracks().forEach(track => pc.addTrack(track, stream)))

// Start circles
requestAnimationFrame(() => drawCircle(canvasOne.getContext('2d'), '#006699', 0))
requestAnimationFrame(() => drawCircle(canvasTwo.getContext('2d'), '#cf635f', 0))
